...
class EnemySprite(Sprite):  # 敌方战机
    ...
    def update(self):
        ...
        if self.bullettype == 0:
            ...
            self.timer2 += 0.01
            if self.timer > 1:
                bullet = EnemyBulletSprite("images/bullet/enemy_bullet2.png", [self.rect.x + 40, self.rect.y + 39.5],
                                           -2, -1)
                bullet2 = EnemyBulletSprite("images/bullet/enemy_bullet2.png", [self.rect.x + 120, self.rect.y + 39.5],
                                            -2, 1)
                self.bulletgroup.add(bullet)
                self.bulletgroup.add(bullet2)
                self.timer = 0
            if self.timer2 > 4:  # BOSS发射导弹
                bullet = EnemyBulletSprite("images/bullet/boss_bullet.png", [self.rect.x + 80, self.rect.y + 39.5], -2,
                                           0)
                self.bulletgroup.add(bullet)
                self.timer2 = 0
...
def main():
    ...
    playergroup = Group()
    playersprite = PlayerSprite("images/hero/hero_b_03.png", [0, 0, 122, 105], [bgsprite.rect.w / 2 - 61, 600], 5,
                                screen)
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == CREATE_ENEMY:
                enemy = EnemySprite("images/enemy/enemy2.png", [random.randint(0, 410), -50], 2, screen,
            ...
            if event.type == CREATE_BULLET_HELP:
                bullethelp = BulletHelp("images/hero/supply.png", [0, 0, 40, 41], [random.randint(0, 410), -50], 2,
                                        screen, random.randrange(-1, 1, 2))
